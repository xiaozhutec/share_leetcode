...
class Solution(object):
    ...
    3 状态转移
        ...
        3.1 如果dp[j] + 1 > dp[i]，说明最长递增子序列的长度增加了，dp[i] = dp[j] + 1，长度增加，数量不变 count[i] = count[j]
        3.2 如果dp[j] + 1 == dp[i]，说明最长递增子序列的长度并没有增加，但是出现了长度一样的情况，数量增加 count[i] += count[j]
            ...
            for j in range(0, i):
                if nums[i] > nums[j]:
                    if dp[i] < dp[j] + 1:
                        ...
                        print(i, j, dp[i], dp[j], count[i], count[j])
                        count[i] += count[j]
                # 记录最长子序列长度
                if max_len < dp[i]:
                    max_len = dp[i]
